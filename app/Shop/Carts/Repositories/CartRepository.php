...
class CartRepository extends BaseRepository implements CartRepositoryInterface
{
    ...
    public function getCartItemsTransformed() : Collection
    {
        return $this->getCartItems()->map(function ($item) {
            ...
            if(!empty($item->product->vendor_id)){
                $vendorRepo = new VendorRepository(new Vendor());
                $vendor = $vendorRepo->findVendorById($item->product->vendor_id);
                $product->vendor_company = $vendor->name;
            }
