...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $vendor_msg =  DB::table('vendor_msg')
                    ...
                    ->orderBy('vendor_msg.id', 'desc')
                    ->limit(3)
                    ->get();
                    
        $vendor_msg_count=DB::table("vendor_msg")->where('vendor_id', $id)->where('status', 'replied')->where('read_status', '1')->count();
        }else{
           $vendor_msg =  DB::table('vendor_msg')
                        ->join('vendors', 'vendors.id', '=', 'vendor_msg.vendor_id')
                        ->select('vendors.first_name','vendors.last_name','vendors.avatar AS vendor_image','vendor_msg.*')
           ...
           $vendor_msg_count=DB::table("vendor_msg")->where('reply_id', NULL)->where('read_status', '0')->count();
        ...
        if(isset(auth('employee')->user()->id)){
          $idd=auth('employee')->user()->id;
           $admintype =  DB::table('employees')->where('id', $idd)->first();
           $type= $admintype->type;
        }
