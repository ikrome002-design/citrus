...
class GlobalTemplateServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        view()->composer([
            'layouts.vendor.app',
            'layouts.vendor.sidebar'

        ], function ($view) {
            $view->with('vendor', Auth::guard('vendor')->user());
        });
        ...
        view()->composer([
            ...
            $breadcumb = [
                ["name" => "Dashboard", "url" => route("vendor.dashboard"), "icon" => "fa fa-dashboard"],
            ];
            $paths = request()->segments();
            if (count($paths) > 1) {
                foreach ($paths as $key => $pah) {
                    if ($key == 1)
                        $breadcumb[] = ["name" => ucfirst($pah), "url" => request()->getBaseUrl() . "/" . $paths[0] . "/" . $paths[$key], 'icon' => config("module.vendor." . $pah . ".icon")];
                    elseif ($key == 2)
                        $breadcumb[] = ["name" => ucfirst($pah), "url" => request()->getBaseUrl() . "/" . $paths[0] . "/" . $paths[1] . "/" . $paths[$key], 'icon' => config("module.vendor." . $pah . ".icon")];
                }
            }
            $view->with(
                [
    ...
    private function isAdmin(Employee $employee)
    {
        ...
        return $employeeRepo->hasRole('staff');
    }
}
