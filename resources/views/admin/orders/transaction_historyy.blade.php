    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
      <li class="nav-item">
        ...
        <!-- <select name="cars" id="cars">
             <option value="{{ route('admin.orders.transaction_report') }}">Select Vendor </option>
               @foreach($order_type as $orderr)
                 <option value="{{ $orderr->name }}"><a class="sub-item" href="{{ route('admin.orders.transaction_reportt', $orderr->id) }}" >{{ $orderr->name }}</a></option>
                @endforeach 
        </select> -->
       <?php //echo request()->segment(4); die;?>
       <ul id="nav" >
       @foreach($order_type as $orderr)
