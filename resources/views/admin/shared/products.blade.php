            ...
            @foreach ($products as $product)
                <tr>
                    ...
                    <form action="{{ route('admin.products.destroy', $product->id) }}" method="post" class="form-horizontal">
                        ...
                        <a  href="@if(isset(auth('employee')->user()->id)){{ route('admin.products.edit', $product->id) }} @elseif(isset(auth('vendor')->user()->id)){{ route('products.edit', $product->id) }}@endif" class="btn btn-success btn-sm vendor-product-bt"><i class="fa fa-edit"></i></a>
                    ...
                    <td>
                        @if($product->status == 1)
                                <a  href="@if(isset(auth('employee')->user()->id)){{ route('admin.products.update.unapprove', ['product' => $product->id]) }} @elseif(isset(auth('vendor')->user()->id)){{ route('products.update.unapprove', ['product' => $product->id]) }}@endif" class="btn btn-primary btn-sm vendor-product-bt" onclick="return confirm('Are you sure? Inactive product!')"> <i class="fa fa-eye"></i></a>
                            @else
                                <a  href="@if(isset(auth('employee')->user()->id)){{ route('admin.products.update.approve', ['product' => $product->id]) }}@elseif(isset(auth('vendor')->user()->id)){{ route('products.update.approve', ['product' => $product->id]) }}@endif" class="btn btn-danger vendor-product-bt" onclick="return confirm('Are you sure? Active product!')"><i class="fa fa-eye-slash"></i></a>
                            @endif
