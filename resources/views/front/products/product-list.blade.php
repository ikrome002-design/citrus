...
<div class="row">
    <div class="col-sm-6">
       <div class="shorting_detail_left"> Showing <span id="product-from-val">{{$product_from+1}}</span> - <span id="product-to-val">{{$product_to}}</span> item(s) of {{count($all_products)}} item(s)</div>
    </div>
    <div class="col-sm-6 text-left text-sm-right">
        <form method="get" action="" id="product-sort-form">
            Sort by: 
            <div class="sort-by-product-bx">
            <select class=" sort-by-product form-control" name="sort" id="product-sort-form-field">
...
@if(!empty($products) && !collect($products)->isEmpty())
                    ...
                    <div class="product-wrap p-0 border-1 pb-3">
                        <figure><a href="{{ route('front.get.product', str_slug($product->slug)) }}">
                            ...
                            <div class="d-flex quick-links" role="group">
                                ...
                                <button type="button" class="btn btn-warning add-to-wishlist-btn product-wishlist-btn myBtnCls" product-id="{{ $product->product_id }}"><i class="fa fa-heart-o"></i></button>
                        ...
                        <div class="px-3">
                            <a href="{{ route('front.get.product', str_slug($product->slug)) }}"><h2 class="product-tittle" data-toggle="tooltip" title="{{ $product->productname }}">{{ $product->productname }}</h2></a>
    ...
    @if(isset($all_products))
        <?php //echo count($all_products)/$product_perpage;?>
        @include('front.products.pagination', ['pages' => count($all_products)/$product_perpage])
    @endif
    @else
